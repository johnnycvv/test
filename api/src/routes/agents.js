...
SELECT id, email, display_name, role, extension, sip_username, sip_password,               status, is_active, created_at        FROM users        WHERE tenant_id = $1        ORDER BY display_name ASC,
[req.tenantId]
);
...
const result = await db.query(
INSERT INTO users (tenant_id, email, display_name, role, extension, sip_username, sip_password, password_hash, is_active)        VALUES ($1,$2,$3,$4,$5,$6,$7,$8,true) RETURNING id, email, display_name, role, extension, sip_username,
[req.tenantId, email, displayName, role, extension || null, sipUsername, sipPassword, hash]
);
res.status(201).json({ ...result.rows[0], temporaryPassword });
} catch (err) {
console.error('[Agents] invite error:', err.message);
res.status(500).json({ error: 'Server error' });
}
});
...
return res.status(400).json({ error: 'Agent has no SIP credentials.' });
}
res.json({
