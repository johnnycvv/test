...
router.get('/', async (req, res) => {
  try {
    const result = await db.query('SELECT id, email, display_name, role, extension, sip_username, sip_password, status, is_active, created_at FROM users WHERE tenant_id = ' + ' ORDER BY display_name ASC', [req.tenantId]);
...
router.post('/invite', requireRole('admin', 'supervisor'), async (req, res) => {
  try {
    ...
    const sipUsername = email.split('@')[0].replace(/[^a-z0-9]/gi, '').toLowerCase() + Math.floor(Math.random() * 1000);
    const sipPassword = uuid().replace(/-/g, '').slice(0, 16);
    const result = await db.query('INSERT INTO users (tenant_id, email, display_name, role, extension, sip_username, sip_password, password_hash, is_active) VALUES (' + ',,,,,,,,true) RETURNING id, email, display_name, role, extension, sip_username', [req.tenantId, email, displayName, role, extension || null, sipUsername, sipPassword, hash]);
...
router.patch('/:id', requireRole('admin', 'supervisor'), async (req, res) => {
  try {
    ...
    await db.query('UPDATE users SET ' + fields.join(',') + ' WHERE id=' + '$' + i++ + ' AND tenant_id=' + '$' + i, vals);
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: 'Server error' }); }
});
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    await db.query('UPDATE users SET status=' + ' WHERE id=' + ' AND tenant_id=' + '', [status, req.params.id, req.tenantId]);
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: 'Server error' }); }
});
router.patch('/:id/password', requireRole('admin'), async (req, res) => {
  try {
    ...
    await db.query('UPDATE users SET password_hash=' + ' WHERE id=' + ' AND tenant_id=' + '', [hash, req.params.id, req.tenantId]);
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: 'Server error' }); }
});
router.delete('/:id', requireRole('admin'), async (req, res) => {
  try {
    await db.query('DELETE FROM users WHERE id=' + ' AND tenant_id=' + '', [req.params.id, req.tenantId]);
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: 'Server error' }); }
});
router.get('/:id/qr-token', requireRole('admin'), async (req, res) => {
  try {
    const result = await db.query('SELECT * FROM users WHERE id=' + ' AND tenant_id=' + '', [req.params.id, req.tenantId]);
...
router.get('/:id/sip-config', requireRole('admin', 'supervisor'), async (req, res) => {
  try {
    const agentResult = await db.query('SELECT u.display_name, u.extension, u.sip_username, u.sip_password, st.registrar, st.username AS trunk_user, st.password AS trunk_pass, st.realm, st.ws_uri FROM users u LEFT JOIN sip_trunks st ON st.tenant_id = u.tenant_id AND st.is_active = true WHERE u.id = ' + ' AND u.tenant_id = ' + ' ORDER BY st.created_at ASC LIMIT 1', [req.params.id, req.tenantId]);
    const agent = agentResult.rows[0];
    if (!agent) return res.status(404).json({ error: 'Agent not found' });
    if (!agent.registrar) return res.status(404).json({ error: 'No active SIP trunk configured. Go to Dashboard → SIP Trunks and add one first.' });
    ...
    if (!username || !password) return res.status(400).json({ error: 'Agent has no SIP credentials.' });
  ...
  } catch (err) { console.error('[Agents] sip-config error:', err.message); res.status(500).json({ error: 'Server error' }); }
});
