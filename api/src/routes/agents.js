...
router.get('/:id/sip-config', requireRole('admin', 'supervisor'), async (req, res) => {
  try {
    const ar = await db.query('SELECT u.display_name, u.extension, u.sip_username, u.sip_password, st.registrar, st.username AS trunk_user, st.password AS trunk_pass, st.realm FROM users u LEFT JOIN sip_trunks st ON st.tenant_id = u.tenant_id AND st.is_active = true WHERE u.id = $1 AND u.tenant_id = $2 ORDER BY st.created_at ASC LIMIT 1', [req.params.id, req.tenantId]);
    const agent = ar.rows[0];
    if (!agent) return res.status(404).json({ error: 'Agent not found' });
    if (!agent.registrar) return res.status(404).json({ error: 'No active SIP trunk configured. Add one in Dashboard first.' });
