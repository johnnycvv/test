...
try {
const result = await db.query('SELECT id, email, display_name, role, extension, sip_username, sip_password, status, is_active, created_at FROM users WHERE tenant_id = $1 ORDER BY display_name ASC', [req.tenantId]);
...
if (!agent) return res.status(404).json({ error: 'Agent not found' });
if (!agent.registrar) return res.status(404).json({ error: 'No active SIP trunk configured. Go to Dashboard and add a SIP trunk first.' });
